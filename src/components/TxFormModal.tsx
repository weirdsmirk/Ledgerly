...
function TxFormModal({ open, editingId, onClose, onSaved }: {
  ...
  useEffect(() => {
    ...
    if (editingId === null) {
      ...
      setAccName(''); setAccType('checking'); setAccError(null);
    } else {
  ...
  // With no accounts there's nothing to pick — surface the quick-add form.
  useEffect(() => {
    if (open && noAccounts) setAccFormOpen(true);
  }, [open, noAccounts]);
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toasts.push(isEdit ? 'Transaction updated' : 'Transaction saved');
      onSaved();
    } catch (err) {
        ...
        <form onSubmit={submit} className="form">
          ...
          <div className="form-grid-2">
            ...
            <div>
              ...
              <div className="acc-inline-toggle">
                <button type="button" className="btn btn-outline btn-xs" onClick={() => { setAccFormOpen((v) => !v); setAccError(null); }}>
                  {accFormOpen ? 'Cancel new account' : '+ New account'}
                </button>
              </div>
            </div>
          </div>
            ...
            <div className="acc-inline">
              <div className="form-grid-2">
                <TextInput
                  ...
                  onKeyDown={(e) => { if (e.key === 'Enter') { e.preventDefault(); createAccountInline(); } }}
                />
                <Select label="Type" value={accType} onChange={(e) => setAccType(e.target.value)}>
                  {ACCOUNT_TYPES.map((t) => (<option key={t.value} value={t.value}>{t.label}</option>))}
                </Select>
              </div>
              {accError && <FieldError>{accError}</FieldError>}
              <div className="modal-actions">
                <Button type="button" variant="outline" className="btn-xs" onClick={() => setAccFormOpen(false)}>Cancel</Button>
                <Button type="button" variant="primary" className="btn-xs" onClick={createAccountInline} disabled={creatingAcc}>
          ...
          {noAccounts && <FieldError>No accounts exist — add one with “+ New account” above to get started.</FieldError>}
